...
test.describe('Library Demo Page Preview', () => {
    test('should compile without asset errors in standard mode', async ({ page }) => {
        // Go directly to the preview route for LibraryDemoPage in standard mode
        const tsPath = 'pages/UEFA_Library/LibraryDemoPage.ts';
        await page.goto(`/preview?path=${encodeURIComponent(tsPath)}&mode=standard`);

        // Wait for preview to attempt compilation
        await page.waitForTimeout(1000); // Give it a brief moment to start
