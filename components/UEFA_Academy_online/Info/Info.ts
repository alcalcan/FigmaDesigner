...
export class Info extends BaseComponent {
  async create(props: ComponentProps): Promise<SceneNode> {
      ...
      "children": [
        {
          ...
          "layoutProps": { "width": 64, "height": 64, "relativeTransform": [[1, 0, 488], [0, 1, 0]], "parentIsAutoLayout": true, "layoutPositioning": "ABSOLUTE" }
        },
        {
          "type": "FRAME",
          "name": "Frame 1615",
    ...
    // Override dynamic text
    const titleNode = this.findChildByName(root, "Don’t miss out!") as TextNode;
    if (titleNode && props.title) {
      titleNode.characters = props.title;
    }

    const infoNode = this.findChildByName(root, "Click on the bell icon") as TextNode;
    if (infoNode && props.text) {
      infoNode.characters = props.text;
    }

    // Hide the rest of the text that was specific to the old info node if new text is provided
    if (props.text) {
      const bellIcon = this.findChildByName(root, "Notifications / notifications-active");
      if (bellIcon) bellIcon.visible = false;
      const tailText = this.findChildByName(root, "in the top-right corner on the page. Use it to choose what you want to follow and we’ll notify you when new resources are added that match your interests.") as TextNode;
      if (tailText) tailText.visible = false;
    }
  ...
  private findChildByName(node: SceneNode, name: string): SceneNode | null {
