...
async function useSystemDialog(payload, pptInstance) {
    ...
    try {
        ...
        const pathResponse = await fetch('http://127.0.0.1:4000/select-save-path', {
            method: 'POST',
        ...
        if (pathResult.path) {
            ...
            const saveResponse = await fetch('http://127.0.0.1:4000/save-ppt', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
...
async function confirmPptExport() {
    ...
    try {
        ...
        // Send to Bridge
        const response = await fetch('http://127.0.0.1:4000/save-ppt', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                fileName: fileName,
    ...
    pptFileInput.addEventListener('change', async (e) => {
        ...
        try {
            ...
            reader.onload = async () => {
                ...
                try {
                    const response = await fetch('http://127.0.0.1:4000/import-ppt', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                            fileName: file.name,
