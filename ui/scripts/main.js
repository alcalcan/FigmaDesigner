        ...
        batchDeleteBtn.onclick = async () => {
            ...
            try {
                if (files.length > 0) {
                    await fetch('http://127.0.0.1:3001/batch-delete', {
                        method: 'POST',
                ...
                if (comps.length > 0) {
                    await fetch('http://127.0.0.1:3001/batch-delete-components', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                ...
                if (pages.length > 0) {
                    await fetch('http://127.0.0.1:3001/batch-delete-pages', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
        ...
        batchConvertBtn.onclick = async () => {
            ...
            try {
                for (const itemPath of items) {
                    ...
                    await fetch('http://127.0.0.1:3001/generate-from-json', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                            path: itemPath,
        ...
        generateSelectedBtn.onclick = async () => {
            ...
            try {
                const response = await
                    fetch(`http://127.0.0.1:3001/read?path=${encodeURIComponent(selectedFilePath)}`);
                if (!response.ok) {
                ...
                if (assetPaths.size > 0) {
                    await Promise.all(Array.from(assetPaths).map(async (assetRef) => {
                        try {
                            ...
                            const listRes = await
                                fetch(`http://127.0.0.1:3001/read-asset?project=&path=${encodeURIComponent(fullAssetPath)}`);
        ...
        convertAllBtn.onclick = async () => {
            ...
            try {
                const isSimplified = simplifiedCodeCheckbox.checked;
                const isProcedural = proceduralGenCheckbox.checked;
                const response = await fetch('http://127.0.0.1:3001/generate-folder-to-code', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
        ...
        async function loadFiles() {
            ...
            try {
                const response = await fetch(`http://127.0.0.1:3001/list?t=${Date.now()}`);
        ...
        async function loadComponents() {
            ...
            try {
                const response = await fetch(`http://127.0.0.1:3001/list-components?t=${Date.now()}`);
                if (!response.ok) throw new Error("Failed");
            ...
            function createRow(item, paddingLeft) {
                ...
                const showMoveInput = (e) => {
                    ...
                    const save = async () => {
                        ...
                        try {
                            const resp = await fetch('http://127.0.0.1:3001/move', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({
                ...
                delBtn.onclick = async (e) => {
                    ...
                    try {
                        let url = item.type === 'file' ? 'http://127.0.0.1:3001/delete' :
                            'http://127.0.0.1:3001/delete-component';
            ...
            Object.keys(groups).sort().forEach(projName => {
                ...
                delGroupBtn.onclick = async (e) => {
                    ...
                    if (container.id === 'file-list') {
                        ...
                        try {
                            const resp = await fetch('http://127.0.0.1:3001/delete', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                        ...
                        try {
                            const resp = await fetch('http://127.0.0.1:3001/delete-component-folder', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
        ...
        window.onmessage = async (event) => {
            ...
            if (msg.type === 'capture-png-result-packet') {
                try {
                    ...
                    const response = await fetch('http://127.0.0.1:3001/save-png', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                ...
                if (processAuto) {
                    ...
                    const checkSaves = async () => {
                        ...
                        try {
                            ...
                            await fetch('http://127.0.0.1:3001/generate-folder-to-code', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({
                                    folder: batchFolder,
            ...
            if (msg.type === 'capture-preview-result-packet') {
                try {
                    ...
                    if (isClipboardCapture) {
                        ...
                        const isProcedural = document.getElementById('procedural-gen-checkbox-capture').checked;
                        const response = await fetch('http://127.0.0.1:3001/generate-clipboard', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                                packet: packet,
                    ...
                    const response = await fetch('http://127.0.0.1:3001/generate-code-preview', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                            packet: packet,
